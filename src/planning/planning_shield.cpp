...
PlanningShield::PlanningShield(
    ...
    : original_spec_str_(spec_formula_str),
 ...
 * @return true if the trajectory is safe, false otherwise
 */
PlanningShield::VerificationResult PlanningShield::verify2(const autoware_planning_msgs::msg::Trajectory& planning_msg, 
        const nlohmann::json& recorded_data) {
    ...
    auto internal_result = doVerify2(
        planning_msg,
        recorded_data,
        closest_point_id,
        ego_size,
    ...
    auto verify_func = [this, &recorded_data, closest_point_id, &ego_size, &ego_center_offset](const autoware_planning_msgs::msg::Trajectory& traj) -> bool {
        // Re-verify the trajectory
        auto result = this->doVerify2(traj, recorded_data, closest_point_id, ego_size, ego_center_offset);
        return result.is_safe;
    };
 ...
 * @brief Internal verification that returns collision information
 */
PlanningShield::InternalVerificationResult PlanningShield::doVerify2(
        const autoware_planning_msgs::msg::Trajectory& planning_msg,
        const nlohmann::json& recorded_data,
        size_t closest_point_id,
        const glm::vec2& ego_size,
    ...
    fqltl::PlannedTraj planned_traj;

    float cumulative_time = 0.0f;
    for (size_t i = closest_point_id; i < planning_msg.points.size(); ++i) {
        const auto& point = planning_msg.points[i];
        glm::vec3 pos_curr = extractObj3DPosition(point);

        if (i > closest_point_id) {
            const auto& prev_point = planning_msg.points[i - 1];
            glm::vec3 pos_prev = extractObj3DPosition(prev_point);
            float speed_prev = std::sqrt(prev_point.longitudinal_velocity_mps * prev_point.longitudinal_velocity_mps +
                                         prev_point.lateral_velocity_mps * prev_point.lateral_velocity_mps);
            float speed_curr = std::sqrt(point.longitudinal_velocity_mps * point.longitudinal_velocity_mps +
                                         point.lateral_velocity_mps * point.lateral_velocity_mps);
        ...
        fqltl::PlannedTrajPoint path_point;
        path_point.position = pos_curr;
        path_point.heading = static_cast<float>(euler[2]);  // yaw in degrees
        path_point.long_vel = point.longitudinal_velocity_mps;
        path_point.lat_vel = point.lateral_velocity_mps;
        path_point.acc = point.acceleration_mps2;
        path_point.time_from_start = cumulative_time;
        planned_traj.points().emplace_back(path_point);
    }

    if (planned_traj.points().empty()) {
        return {true, {}}; // no trajectory points to verify
    }
    ...
    std::vector<fqltl::PerceivedObject> perceived_objs;

    // Iterate over each detected object
    for (const auto& obj : perception_data["objects"]) {
        fqltl::PerceivedObject perceived_obj;

        std::string obj_id = obj["id"];
        // replace all spaces by underscores
        obj_id = std::regex_replace(obj_id, std::regex(" "), "_");
        perceived_obj.set_id(obj_id);
        perceived_obj.set_probability(obj["existence_prob"]);
        perceived_obj.set_position(jsonPointToVector3(obj["pose"]["position"]));
        perceived_obj.set_heading(obj["pose"]["rotation"]["z"]);  // yaw in degrees
        perceived_obj.set_velocity(jsonPointToVector3(obj["twist"]["linear"]));
        
        // check initial distance, and skip objects that are too far away
        float initial_distance = glm::length(planned_traj.points()[0].position - perceived_obj.position());
        if (initial_distance > distance_bound_) {
            continue;
        }
        ...
        if (obj["shape"]["type"] == "box") {
            float length = obj["shape"]["size"]["x"];
            float width = obj["shape"]["size"]["y"];
            perceived_obj.local_vertices().push_back(glm::vec2(length / 2, width / 2));
            perceived_obj.local_vertices().push_back(glm::vec2(-length / 2, width / 2));
            perceived_obj.local_vertices().push_back(glm::vec2(-length / 2, -width / 2));
            perceived_obj.local_vertices().push_back(glm::vec2(length / 2, -width / 2));
        } else if (obj["shape"]["type"] == "polygon") {
            for (const auto& pt : obj["shape"]["footprint"]) {
                perceived_obj.local_vertices().push_back(glm::vec2(pt["x"], pt["y"]));
            }
        } else {
            // Cylinder or unsupported shape - skip
            continue;
        ...
        // Iterate over each predicted path of this object
        for (const auto& predicted_path : obj["predict_paths"]) {
            fqltl::PredictedTravelPath predicted_path_obj;
            predicted_path_obj.set_confidence(predicted_path["confidence"]);
            predicted_path_obj.set_time_step(predicted_path["time_step"]);
            
            for (const auto& pt : predicted_path["path"]) {
                fqltl::PredictedTravelPathPoint path_point;
                path_point.position = jsonPointToVector3(pt["position"]);
                path_point.heading = pt["rotation"]["z"];  // yaw in degrees
                predicted_path_obj.path_points().emplace_back(path_point);
            }

            if (predicted_path_obj.path_points().empty()) {
                continue;
            }
            perceived_obj.predicted_paths().emplace_back(predicted_path_obj);
        }

        perceived_objs.emplace_back(perceived_obj);
    }

    fqltl::PlanningRuntimeData runtime_data(ego_size, ego_center_offset, perceived_objs, planned_traj);
    
    bool result = fqltl::FqltlEvaluator::evaluate("[] forall obj. forall path@obj. ((confidence(path) >= 0.1 /\\ time <= 3.0) -> ~ collision(obj, path))", 
        runtime_data, collision_index_set);

    std::vector<size_t> collision_indices(collision_index_set.begin(), collision_index_set.end());
    // need to plus the closest_point_id offset to align with the original trajectory point indices
    for (size_t& idx : collision_indices) {
        idx += closest_point_id;
    }
